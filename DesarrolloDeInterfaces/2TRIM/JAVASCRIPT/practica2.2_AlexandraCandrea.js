...
//El Error estaba en que en el siguiente console.log faltaba pasarle el segundo parametro
//a la funcion potencia (el 3)
...
//estaba mal escrito "length" y al recorrer con el for hasta 
// <= recorria valores inexistentes
let notas = [5, 7, 9];
...
/*
faltaba añadir com oargumentos a Modulo "positivos", quitar el segundo
.this de positivos ya que daba error y faltaba en la funcion media
el this.examen, sin el , el progrtama no sabe a que examen se refiere
*/
Modulo.prototype.media = function() {
  return this.examen * 0.30 + this.tareas * 0.20 + this.practicas * 0.45 + this.positivos * 0.05;
};

let diEj4 = new Modulo("Desarrollo de Interfaces", 7, 8, 9, 10);
...
//la sintaxis no era correcta debe quedar declarado nombre para 
//recorrer la lista.
...
//faltaba meter el aprametro a la siguiente funcion esMayorDeEdad
console.log("16 es mayor de edad?", esMayorDeEdad(16));
...
// faltaba el this. en saldo al sumar la cantidad
...
function filtrarPares(numeros) {
  let pares = [];
  for (let i = 0; i < numeros.length; i++) {
    //no es correcto comaprar con "=", asi comparamos tipo y valor
    if (numeros[i] % 2 === 0) {
...
//no se metia ningun valor por parametro en factorial
...
function mostrarNotas(listaAlumnos) {
    // el for al tener <= que el length recorria valores inexistentes
...
//faltaba el tercer parametro de la funcion que indica que operacion hacer
let resultadoEj11 = operar(10, 5, "sumar");
...
function clasificarNumero(num) {
  if (num > 0) {
    ...
    // hay que hacer una comparacion === para comaprar tipo y valor a la vez
  } else if (num === 0) {
    console.log("Cero");
  }
}
...
function aMayusculas(lista) {
  ...
  for (let texto of lista) {
    //el metodo toUpperCase estaba mal escrito 
...
// ponia indice en vez de indiceej13, y al principio 14 en vez de 13
...
function nombreMes(mes) {
  ...
  return meses[mes-1];
}//con el inndice empieza en 0 hay que restare uno al mes
...
function dividir(a, b) {
  try {
    ...
    return a / b;
    //la sintaxis correcta el el catch es con parentesis
  } catch (error) {
...
// faltaba poner el parametro en el segundo metodo parOImpar
